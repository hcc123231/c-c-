...
typedef int (*msg_handler)(char* msg,int length,char* response,int fd);
...
int kvs_request(struct conn* c,int fd){
    char resp[1024]={0};
    kvs_handler(c->rbuf,strlen(c->rbuf),resp,fd);
...
kvs_rbtree_t* rbtrees[100]={0};//暂定100个用户
kvs_rbtree_t* hash[100000]={0};
mempool_t* memp[100000]={0};
void recv_callback(int fd)
{
    ...
    else if(ret==0)
    {
        ...
        kvs_dest_kvsengine(hash[fd],fd);
        mempool_create(memp[fd],sizeof(rbtree_node));
        conn_list[fd].fd=0;
...
{   
    int i=0;
    for(i=0;i<sizeof(rbtrees)/sizeof(rbtrees[0]);i++){
        if(rbtrees[i]==NULL)break;
    }
    rbtrees[i]=(kvs_rbtree_t*)malloc(sizeof(kvs_rbtree_t));
    if(rbtrees[i]==NULL)return;

    struct sockaddr_in cliaddr;
    int len=sizeof(cliaddr);
    int acfd=accept(fd,(struct sockaddr*)&cliaddr,&len);
    memp[acfd]=(mempool_t*)malloc(sizeof(mempool_t));
    if(memp[acfd]==NULL)return;
    
    mempool_create(memp[acfd],sizeof(rbtree_node));
    kvs_init_kvsengine(rbtrees[i],acfd);
    if(acfd==-1)
    {
